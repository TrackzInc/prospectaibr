...
const defaultXAxisFormat = (v: number | Date) => new Date(v).toLocaleDateString('pt-BR', {
...
export const AreaChartXS: React.FC<ComponentProps> = ({
  ...
  return (
    <AreaChart id={id} data={data} width={width} height={height}
      ...
      xAxis={<LinearXAxis type="time" tickSeries={showXAxisTicks ? <LinearXAxisTickSeries label={<LinearXAxisTickLabel format={xAxisFormat} fill={themeColors.axisTickColor} fontSize={10}/>} tickSize={10}/> : undefined} axisLine={null}/>}
      yAxis={<LinearYAxis axisLine={null} tickSeries={showYAxisTicks ? <LinearYAxisTickSeries tickSize={10} label={{fill:themeColors.axisTickColor, fontSize: 10}}/> : <LinearYAxisTickSeries line={null} label={null} tickSize={0}/>}/>}
